/**
 * Datenzugriff für das Innehalten-Modul – die einzige Stelle, die dafür SQL
 * kennt.
 *
 ...
 * SYNC_TABLES in src/features/pause/model.js.
 *
...
export function createPauseRepository(db) {
  const selectById = db.prepare('SELECT * FROM pause_records WHERE id = ?');
  const selectWireById = db.prepare(
    'SELECT kind, id, payload, createdAt, updatedAt, deletedAt FROM pause_records WHERE id = ?'
  );
  ...
  const insertRecord = db.prepare(`
    INSERT INTO pause_records (id, kind, payload, createdAt, updatedAt, deletedAt, serverUpdatedAt)
  ...
  const deleteAll = db.prepare('DELETE FROM pause_records');
  const countRecords = db.prepare(
    'SELECT COUNT(*) AS total, COUNT(deletedAt) AS deleted FROM pause_records'
  );
   ...
   *    Client zwingend bekommen – siehe Kommentar in routes/pause.js.
