import { RECORD_KINDS } from '../clarity/repository.js';

/**
 * Endpunkte des Klarblick-Moduls.
 *
 ...
 *  - Zu `DELETE /clarity/all` siehe unten: die einzige Stelle der Anwendung,
 *    die hart löscht.
 *
...
export default async function clarityRoutes(app) {
  /**
  ...
  app.setErrorHandler((error, request, reply) => {
    if (error.validation) {
      request.log.warn({ code: 'INVALID_INPUT', url: request.url }, 'Klarblick: Anfrage abgelehnt');
  ...
  app.post(
    ...
    async (request) => {
      ...
      const { settled, rejected, applied } = app.clarity.applyIncoming(incoming, Date.now());

      const changes = app.clarity.listChangedSince(since);
      ...
      for (const { kind, id } of rejected) {
        ...
        const correction = app.clarity.getById(id);
  ...
  app.delete('/all', async (request) => {
    const deleted = app.clarity.wipeAll();
    request.log.info({ deleted }, 'Klarblick: alle Daten gelöscht');
    return { deleted };
  });
}
