...
import { wipeAllClarityData } from './claritySync.js';
import { SYNC_TABLES } from './model.js';
import { deleteRecord, useRecordCounts, useRecords } from './useClarity.js';
import { useClaritySync } from './useClaritySync.js';
import classes from './Clarity.module.scss';

/**
 * Prüfseite auf der Datenschicht, erreichbar unter /clarity/debug.
 *
 ...
 * Entwurf der Oberfläche – die steht unter /clarity.
 *
...
export default function ClarityDebugPage() {
  const { status, lastSyncedAt, lastResult, error, sync } = useClaritySync();
  ...
  return (
    <Container size="sm" px={0} className={classes.page}>
      <Stack gap="lg">
        <div>
          <Title order={2}>Klarblick — Datenschicht</Title>
...
function DangerCard() {
  ...
  async function wipe() {
    setBusy(true);
    try {
      const { deletedOnServer } = await wipeAllClarityData();
