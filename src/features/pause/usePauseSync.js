...
import { syncPause } from './pauseSync.js';

/**
 * Hängt den Innehalten-Sync an TanStack Query – dieselbe Konstruktion wie beim
...
export const pauseSyncQueryKey = ['pause', 'sync'];

export function usePauseSync() {
  const { online } = useNetwork();

  const query = useQuery({
    queryKey: pauseSyncQueryKey,
    queryFn: ({ signal }) => syncPause({ signal }),
