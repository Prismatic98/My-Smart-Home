...
/**
 * Datenzugriffsschicht des Innehalten-Moduls – die einzige Stelle, die Dexie
 * direkt anspricht. Komponenten nutzen die Hooks aus usePause.js, der Sync
...
const LAST_SYNCED_KEY = 'pause:lastSyncedAt';
...
function newId() {
  ...
  return `pause-${Date.now()}-${Math.random().toString(16).slice(2, 10)}`;
}
...
function tableOf(table) {
  ...
  if (!store || !SYNC_TABLES.includes(table)) {
    throw new Error(`Unbekannte Innehalten-Tabelle: ${table}`);
  }
  return store;
}
...
export async function updateRecord(table, id, changes) {
  ...
  if (modified === 0) {
    throw new Error(`Innehalten-Datensatz ${id} existiert nicht (mehr).`);
  }
}
