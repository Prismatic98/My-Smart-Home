 ...
 *      aus Dexie holen.
 *   2. Beides an POST /pause/sync schicken.
 ...
 * im Takt aus usePauseSync.js. Das ist der Grund, warum ein Regler, den man
 ...
 * ohnehin drankommt.
 */
export async function syncPause({ signal } = {}) {
  const since = await getLastSyncedAt();
  const pushed = await listDirtyRecords();

  const response = await backendRequest('/pause/sync', {
    method: 'POST',
 ...
 */
export async function wipeAllPauseData({ signal } = {}) {
  const response = await backendRequest('/pause/all', { method: 'DELETE', signal });
  await wipeLocalData();
